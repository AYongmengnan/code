...
def parse_detail(url):
    print(url)
    url_exists = r.sismember('detail_url', url)
    if url_exists:
        return
    response = requests.get(url=url,headers=headers)
    html = Selector(text=response.text)
    head_dict = {}
    cates = html.xpath('//div[@class="Navigation"]//text()').getall()
    head_dict['FACTORY_CATE'] = ''.join(''.join(cates).split()).split(':')[-1] if cates else None
    ...
    sku_ids = html.xpath('//tbody[@class="right-tables"]/tr/td[1]/text()').getall() # 全部货号
    ...
    if data:
        da_dict = {}
        for da in data:
            k = da.xpath('td[1]/text()').get().strip()
            v = da.xpath('td[2]/text()').get().strip()
            fields = key_data.get(k)
            if fields:
                da_dict[fields] = v
    # print(da_dict)
    save_data = []
